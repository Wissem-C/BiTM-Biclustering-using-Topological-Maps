...
class BiTM(val nbRowNeuron: Int, val nbColNeuron: Int, datas: RDD[Array[Double]],val arg: String, val topoFactor: TopoFactor) extends Serializable {
  def this(nbRowNeuron: Int, nbColNeuron: Int, datas: RDD[Array[Double]],arg : String) = this(nbRowNeuron, nbColNeuron, datas,arg, BiTMTopoFactor)
  ...
  private var _colNeuronAffectation: Array[Int] = _
  if (arg == "use_saved" && new File("affecolonne.txt").exists() ) {
   
    val lines = Source.fromFile("affecolonne.txt").getLines().toArray
    _colNeuronAffectation = lines.map(_.toInt)
    println(_colNeuronAffectation.mkString(", "))
  } else {
    _colNeuronAffectation = Array.tabulate(_nbDataCol) { i => _rand.nextInt(nbCol) }
  }
  ...
  def training(nbIter: Int) {
    ...
    for (iter <- 0 until nbIter) {
      ...
      val writer = new PrintWriter("affecolonne.txt")
      _colNeuronAffectation.foreach(i => writer.write(i.toString + "\n"))
      writer.close()
  ...
  class BiTMCol(neuronMatrix: Matrix, nbNeurons: Int, nbDataCol: Int, colNeuronAffectation: Array[Int]) extends Serializable {
    ...
    protected def findBestRowNeuron(rowData: Vector): Int = {
      ...
      for (rowNeuronId <- 0 until _nbNeurons) {
  ...
  def affectations(dataset: RDD[Array[Double]]): RDD[Int] = {
    dataset.map(d => Vectors.dense(d)).map(d =>  this.findBestRowNeuron(d))
  }
